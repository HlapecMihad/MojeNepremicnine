...
def scrape_data(url):
    ...
    #price = float(soup.find('h3', class_='price mb-0').get_text(strip=True).replace(' €', '').replace('/mesec','').replace('/dan','').replace('.', '').replace(',','.')) if soup.find('h3', class_='price mb-0') and soup.find('h3', class_='price mb-0') != 'Po dogovoru' else None
    ...
    data = {
        ...
        'cena': price,
        'st_sob': float(rooms.split('-')[0].replace(',', '.').replace('+', '')) if rooms else None,
        'st_spalnic': spalnice,
